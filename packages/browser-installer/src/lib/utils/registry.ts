...
export const hostPlatform = ((): BrowserPlatform => {
  ...
  if (platform === 'darwin') {
    ...
    if (major >= 11) {
      arm64 =
        execSync('/usr/sbin/sysctl -in hw.optional.arm64', {
          stdio: ['ignore', 'pipe', 'ignore'],
        })
          .toString()
          .trim() === '1'
...
export class Registry {
  ...
  windowsExeAndDllDirectories(browserName: BrowserName): string[] {
    ...
    if (browserName === 'firefox')
      return [path.join(browserDirectory, 'firefox')]
    if (browserName === 'webkit') return [browserDirectory]
    return []
  }
  ...
  downloadURL(browserName: BrowserName): string {
    ...
    assert(
      urlTemplate,
      `ERROR: Playwright does not support ${browserName} on ${hostPlatform}`
    )
