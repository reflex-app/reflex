...
export default class Panzoom {
  ...
  _init = () => {
    ...
    Object.entries(parent_styles).forEach(([key, value]) => {
    ...
    Object.entries(child_styles).forEach(([key, value]) => {
  ...
  setTransform = async newTransformValues => {
    ...
    for (let key in newTransformValues) {
      ...
      if (isNaN(value))
        throw new Error(`Value passed in for ${key} is not a number.`);
  ...
  scaleToFit = async () => {
    ...
    const childExceedsParent =
      childWidth >= parentWidth || childHeight >= parentHeight;
  ...
  scaleToFitEl = domElement => {
    ...
    // If the child is outside of the parent, scale it down
    const childExceedsParent = !!(
      elWidth > parentWidth || elHeight > parentHeight
    );
    ...
    if (childExceedsParent) {
      ...
      if (zoomAspectRatioX < 1 && zoomAspectRatioY < 1) {
        scale(
          Math.min(
            parentWidth / (elWidth / currentScale),
            parentHeight / (elHeight / currentScale)
          )
        );
  ...
  on = (event, cb) => {
    if (!this._eventListener[event])
      throw new Error(`Event not setup ${event} ${cb}`);
